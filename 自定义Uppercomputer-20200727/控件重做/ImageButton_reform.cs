...
namespace 自定义Uppercomputer_20200727.控件重做
{
    ...
    class ImageButton_reform: UI_Library_da.UI_ImageButton
    {
        ...
        {  //标志位复位-并且写入数据库
            ...
            {
                ImageButton_EF button_EF = new ImageButton_EF();//实例化EF
                button_EF.Button_Parameter_modification(((ImageButton_reform)sender).Parent + "-" + ((ImageButton_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((ImageButton_reform)sender).Size.Width + "-" + ((ImageButton_reform)sender).Size.Height
                    });
                startMove = false;
            }
            if (Form2.edit_mode) return;
