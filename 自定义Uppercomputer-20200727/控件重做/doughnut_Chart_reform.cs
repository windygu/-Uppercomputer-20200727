    ...
    class doughnut_Chart_reform : doughnut_Chart
    {
        ...
        {  //标志位复位-并且写入数据库
            if (startMove)
            {
                ...
                button_EF.Button_Parameter_modification(((doughnut_Chart_reform)sender).Parent + "- " + ((doughnut_Chart_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((doughnut_Chart_reform)sender).Size.Width + "-" + ((doughnut_Chart_reform)sender).Size.Height
                    });
                startMove = false;
            }
        }
