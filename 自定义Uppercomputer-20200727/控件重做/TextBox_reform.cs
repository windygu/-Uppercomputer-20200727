    ...
    class SkinTextBox_reform : TextBox
    {
        ...
        {  //标志位复位-并且写入数据库
            if (startMove)
            {
                ...
                button_EF.Button_Parameter_modification(((SkinTextBox_reform)sender).Parent + "- " + ((SkinTextBox_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y +e.Y- clickY).ToString(),
                        size = ((SkinTextBox_reform)sender).Size.Width + "-" + ((SkinTextBox_reform)sender).Size.Height
                    });
                startMove = false;
            }
        }
        ...
        private void numerical_DoubleClick(object sender, EventArgs e)//用户双击控件---进入键盘
        {
            ...
            if (numerical_Classes.资料格式.Trim() == "Hex_16_Bit" || numerical_Classes.资料格式.Trim() == "Hex_32_Bit")
            {
