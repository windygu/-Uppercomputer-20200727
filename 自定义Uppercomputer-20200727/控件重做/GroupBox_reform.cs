    ...
    class GroupBox_reform : UI_Library_da.UI_GroupBox
    {
        ...
        {  //标志位复位-并且写入数据库
            if (startMove)
            {
                ...
                button_EF.Button_Parameter_modification(((GroupBox_reform)sender).Parent + "-" + ((GroupBox_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((GroupBox_reform)sender).Size.Width + "-" + ((GroupBox_reform)sender).Size.Height
                    });
                startMove = false;
            }
        }
