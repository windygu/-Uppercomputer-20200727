...
namespace 自定义Uppercomputer_20200727.控件重做
{
    ...
    class LedBulb_reform : UILedBulb
    {
        ...
        {  //标志位复位-并且写入数据库
            ...
            {
                LedBulb_EF button_EF = new LedBulb_EF();//实例化EF
                button_EF.Button_Parameter_modification(((LedBulb_reform)sender).Parent + "-" + ((LedBulb_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((LedBulb_reform)sender).Size.Width + "-" + ((LedBulb_reform)sender).Size.Height
                    });
                startMove = false;
            }
