...
namespace 自定义Uppercomputer_20200727.控件重做
{
    ...
    class ScrollingText_reform : UIScrollingText
    {
        ...
        {  //标志位复位-并且写入数据库
            if (startMove)
            {
                ScrollingText_EF ScrollingText_EF = new ScrollingText_EF();//实例化EF
                ScrollingText_EF.ScrollingText_Parameter_modification(((ScrollingText_reform)sender).Parent + "-" + ((ScrollingText_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((ScrollingText_reform)sender).Size.Width + "-" + ((ScrollingText_reform)sender).Size.Height
                    });
                startMove = false;
            }
        }
