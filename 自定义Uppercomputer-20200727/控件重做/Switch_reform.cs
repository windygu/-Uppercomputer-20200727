   ...
   /// </summary>
    class Switch_reform : UI_Switch
    {
        ...
        {  //标志位复位-并且写入数据库
            ...
            {
                Switch_EF button_EF = new Switch_EF();//实例化EF
                button_EF.Button_Parameter_modification(((Switch_reform)sender).Parent + "-" + ((Switch_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y + e.Y - clickY).ToString(),
                        size = ((Switch_reform)sender).Size.Width + "-" + ((Switch_reform)sender).Size.Height
                    });
                startMove = false;
            }
            if (Form2.edit_mode) return;
            ...
            switch (pLC)
            {
                case "Mitsubishi"://三菱有二种模式 --在线与仿真
                    if (PLCselect_Form.Mitsubishi.Trim() != "在线访问")//判断用户选定模式
                    {
                        ...
                        else MessageBox.Show("未连接设备：" + Switch_Class.读写设备.Trim(), "Err");//推出异常提示用户
                    }
                    else
                    {
