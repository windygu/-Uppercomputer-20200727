...
namespace 自定义Uppercomputer_20200727.控件重做
{
    ...
    class Button_reform : SkinButton
    {
        ...
        {  //标志位复位-并且写入数据库
            ...
            {
                Button_EF button_EF = new Button_EF();//实例化EF
                button_EF.Button_Parameter_modification(((SkinButton)sender).Parent + "-" + ((SkinButton)sender).Name
                    , new control_location { location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y +e.Y- clickY).ToString(),
                        size = ((SkinButton)sender).Size.Width + "-" + ((SkinButton)sender).Size.Height});
                startMove = false;
            }
            if (Form2.edit_mode) return;
            if (state)
            ...
            switch (pLC)
            {
                case "Mitsubishi"://三菱有二种模式 --在线与仿真
                    if (PLCselect_Form.Mitsubishi.Trim() != "在线访问")//判断用户选定模式
                    {
                        ...
                        else MessageBox.Show("未连接设备：" + Button_Class.读写设备.Trim(), "Err");//推出异常提示用户
                    }
                    else
                    {
