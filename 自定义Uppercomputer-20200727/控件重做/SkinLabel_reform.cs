    ...
    class SkinLabel_reform:SkinLabel
    {
        ...
        {  //标志位复位-并且写入数据库
            if (startMove)
            {
                ...
                button_EF.Button_Parameter_modification(((SkinLabel_reform)sender).Parent + "-" + ((SkinLabel_reform)sender).Name
                    , new control_location
                    {
                        location = (this.Location.X + e.X - clickX).ToString() + "-" + (this.Location.Y +e.Y- clickY).ToString(),
                        size = ((SkinLabel_reform)sender).Size.Width + "-" + ((SkinLabel_reform)sender).Size.Height
                    });
                startMove = false;
            }
        }
