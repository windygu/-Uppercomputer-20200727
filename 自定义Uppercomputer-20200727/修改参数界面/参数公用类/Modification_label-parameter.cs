...
namespace 自定义Uppercomputer_20200727.修改参数界面
{
    ...
    class Modification_label_parameter
    {
        ...
        public Modification_label_parameter(SkinComboBox Status,SkinComboBox typeface
            ...
            , SkinChatRichTextBox content, Button_state state,bool Load_state,string Name,int Button_state)//按钮类
        {
            if(Load_state)//判断是否初始化加载
            {
                ...
                label_Lists.Add(new label_list(0, typeface.SelectedIndex, colour.SelectedIndex, size.SelectedIndex, align_at.SelectedIndex, flicker.SelectedIndex, Name));//添加0状态
                label_Lists.Add(new label_list(1, typeface.SelectedIndex, colour.SelectedIndex, size.SelectedIndex, align_at.SelectedIndex, flicker.SelectedIndex, Name));//添加1状态
            }
            else
            {
                #region 修改控件状态
 ...
 , SkinChatRichTextBox content, string Name)//文本--数值软件类
        ...
        private void ComboBox_typeface(ref SkinComboBox skinCombo)//控件字体
        {
            ...
            skinCombo.SelectedIndex = 328;//默认字体宋体
            skinCombo.SelectedItem = 328;//默认字体宋体
        }
    ...
    class label_list
    {
        public int Status, typeface, colour, size, align_at, flicker;//暂时保存参数
        public string content;
        public label_list(int Status, int typeface,int colour, int size,int align_at,int flicker , string content)
        {
            this.Status = Status; this.typeface = typeface;this.colour = colour;this.size = size;
            this.align_at = align_at;this.flicker = flicker;this.content = content;
