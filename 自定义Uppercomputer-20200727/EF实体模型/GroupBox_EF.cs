...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    ...
    class GroupBox_EF
    {
        ...
        public string GroupBox_Parameter_delete(string ID)//四边形方框类ID删除参数
        {
            ...
            {
                //执行删除按钮类主参数操作
                label_parameter GroupBox_Parameter = model.label_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (GroupBox_Parameter != null)
                {
                    model.label_parameter.Remove(GroupBox_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string label_Parameter_modification(string ID, GroupBox_parameter GroupBox_Parameter, Tag_common_parameters tag_Common_, control_location _Location)//四边形方框类ID修改参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
